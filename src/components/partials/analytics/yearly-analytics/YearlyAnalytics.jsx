...
import { Chart as ChartJS, LineElement, BarElement, PointElement, ArcElement, CategoryScale, LinearScale, Title, Tooltip, Legend } from "chart.js";
...
const YearlyAnalytics = () => {
  ...
  const [year, setYear] = useState(selectedYear.getFullYear());
  ...
  useEffect(() => {
    ...
    const fetchSites = async () => {
      ...
      try {
        ...
        console.error("[YearlyAnalytics] Error fetching sites:", error);
  ...
  useEffect(() => {
    ...
    const fetchDepartments = async () => {
      ...
      try {
        ...
        console.error("[YearlyAnalytics] Error fetching departments:", error);
  ...
  useEffect(() => {
    ...
    const fetchAgents = async () => {
      try {
        ...
        console.error("[YearlyAnalytics] Error fetching agents:", error);
        setAgents([]);
      }
    };
    fetchAgents();
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        const response = await axios.get("https://supportdesk.fskindia.com/analytics/yearly", {
          params,
          headers: {
            Authorization: `Bearer ${token}`,
            Accept: "application/json",
        ...
        console.error("[YearlyAnalytics] Error fetching analytics data:", error);
        setAnalyticsData(null);
      }
    };
    if (token && year) {
      fetchAnalytics();
      analyticsFetchedRef.current = { year, siteId, departmentName, agentEmail };
    }
  }, [year, siteId, departmentName, agentEmail, departments, token]);
    ...
    datasets: [
      {
        ...
        data: analyticsData ? Object.values(analyticsData.monthly_breakdown) : [],
        fill: false,
        borderColor: "#118ab2",
        tension: 0.4,
  ...
  // Chart data for quarterly breakdown (Bar chart)
  const quarterlyChartData = {
    labels: analyticsData ? Object.keys(analyticsData.quarterly_breakdown) : [],
    datasets: [
      {
        label: "Requests per Quarter",
        data: analyticsData ? Object.values(analyticsData.quarterly_breakdown) : [],
    ...
    datasets: [
      {
        ...
        backgroundColor: ["#ef476f", "#ffd166", "#06d6a0", "#118ab2", "#073b4c", "#8338ec"],
        borderColor: "#ffffff",
        borderWidth: 2,
      },
    ],
      ...
      <div className="card-inner">
         <div className="card-title-group pb-3 g-2">
          <div className="card-title">
            <h6 className="title">Yearly Analytics: {analyticsData ? analyticsData.year : year}</h6>
            <p>Analytics for a specific year with monthly breakdown.</p>
          </div>
        ...
        {analyticsData && !showTable ? (
          <Row className="analytic-data-group g-3">
            ...
            <Col sm="6" md="2">
              <div className="analytic-data">
                <div className="title">Busiest Month</div>
                <div className="amount">{analyticsData.busiest_month}</div>
              </div>
            </Col>
            <Col sm="6" md="2">
              <div className="analytic-data">
                <div className="title">Busiest Quarter</div>
                <div className="amount">{analyticsData.busiest_quarter}</div>
              </div>
            </Col>
            <Col sm="6" md="2">
              <div className="analytic-data">
          ...
          <div style={{ maxHeight: "300px", overflowY: "auto", overflowX: "auto" }}>
            <Table responsive className="table-bordered" style={{ minWidth: "600px" }}>
              ...
              <tbody>
                ...
                <tr>
                  <td>Busiest Month</td>
                  <td>{analyticsData.busiest_month}</td>
                </tr>
                <tr>
                  <td>Busiest Quarter</td>
                  <td>{analyticsData.busiest_quarter}</td>
                </tr>
              </tbody>
              <thead>
                <tr>
              ...
              <tbody>
                {analyticsData &&
                  Object.entries(analyticsData.monthly_breakdown).map(([month, count]) => (
                    <tr key={month}>
                      <td>{month}</td>
                      <td>{count}</td>
                    </tr>
              ...
              <thead>
                <tr>
                  <th colSpan="2">Quarterly Breakdown</th>
                </tr>
              </thead>
              <tbody>
                {analyticsData &&
                  Object.entries(analyticsData.quarterly_breakdown).map(([quarter, count]) => (
                    <tr key={quarter}>
                      <td>{quarter}</td>
                      <td>{count}</td>
                    </tr>
