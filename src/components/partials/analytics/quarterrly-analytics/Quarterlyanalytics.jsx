...
const getQuarter = (date) => {
  ...
  return Math.floor(month / 3) + 1; // Months 0-2: Q1, 3-5: Q2, 6-8: Q3, 9-11: Q4
};

const QuarterlyAnalytics = () => {
  const [selectedDate, setSelectedDate] = useState(new Date("2025-06-01"));
  const [year, setYear] = useState(selectedDate.getFullYear());
  const [quarter, setQuarter] = useState(getQuarter(selectedDate));
  ...
  useEffect(() => {
    ...
    const fetchSites = async () => {
      ...
      try {
        ...
        console.error("[QuarterlyAnalytics] Error fetching sites:", error);
  ...
  useEffect(() => {
    ...
    const fetchDepartments = async () => {
      ...
      try {
        const response = await axios.get(
          "https://chatsupport.fskindia.com/departments/",
          {
            headers: {
              Authorization: `Bearer ${token}`,
              Accept: "application/json",
        ...
        console.error(
          "[QuarterlyAnalytics] Error fetching departments:",
          error,
        );
  ...
  useEffect(() => {
    ...
    if (
      ...
      analyticsFetchedRef.current.quarter === quarter &&
    ...
    const fetchAnalytics = async () => {
      try {
        const response = await axios.get(
          "https://supportdesk.fskindia.com/analytics/quarterly",
          {
            params,
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        console.error(
          "[QuarterlyAnalytics] Error fetching analytics data:",
          error,
        );
        setAnalyticsData(null);
      }
    };
    if (token && year && quarter) {
      fetchAnalytics();
      analyticsFetchedRef.current = {
        year,
        quarter,
  ...
  }, [year, quarter, siteId, departmentName, agentEmail, departments, token]);

  // Chart data for monthly breakdown (Line chart)
  const monthlyChartData = {
    labels: analyticsData ? Object.keys(analyticsData.monthly_breakdown) : [],
    datasets: [
      {
        label: "Requests per Month",
        data: analyticsData
          ? Object.values(analyticsData.monthly_breakdown)
          : [],
        fill: false,
        borderColor: "#118ab2",
        tension: 0.4,
  ...
  const priorityChartData = {
    labels: analyticsData
      ? Object.keys(analyticsData.requests_by_priority)
      : [],
    datasets: [
      {
        data: analyticsData
          ? Object.values(analyticsData.requests_by_priority)
          : [],
  ...
  const statusChartData = {
    ...
    datasets: [
      {
        data: analyticsData
          ? Object.values(analyticsData.requests_by_status)
          : [],
  ...
  return (
    <Card className="h-100 w-100 border-0 shadow-sm" style={cardStyles}>
      <div className="card-inner">
        <div className="card-title-group pb-3 g-2 flex-column align-items-start">
          {/* 🔹 TITLE */}
          <div className="card-title w-100">
            <h6 className="title">
              Quarterly Analytics: Q
              {analyticsData ? analyticsData.quarter : quarter}{" "}
              {analyticsData ? analyticsData.year : year} (
              {analyticsData
                ? `${analyticsData.quarter_start} to ${analyticsData.quarter_end}`
                : ""}
              )
            </h6>
            <p>Analytics for a specific quarter with monthly breakdown.</p>
          </div>

          {/* 🔹 FILTERS (NOW FULL WIDTH BELOW TITLE) */}
          <div className="card-tools w-100 mt-3">
        ...
        {analyticsData && !showTable ? (
          <Row className="analytic-data-group g-3">
            ...
            <Col sm="6" md="3">
              <div className="analytic-data">
                <div className="title">Quarter Start</div>
                <div className="amount">{analyticsData.quarter_start}</div>
              </div>
            </Col>
            <Col sm="6" md="3">
              <div className="analytic-data">
                <div className="title">Quarter End</div>
                <div className="amount">{analyticsData.quarter_end}</div>
              </div>
            </Col>
            <Col sm="6" md="3">
              <div className="analytic-data">
                ...
                <div className="amount">
                  {analyticsData.requests_by_status.initiated}
                </div>
              </div>
            </Col>
          </Row>
          ...
          >
            ...
            >
              ...
              <tbody>
                ...
                <tr>
                  <td>Quarter</td>
                  <td>{analyticsData.quarter}</td>
                </tr>
                <tr>
                  <td>Quarter Start</td>
                  <td>{analyticsData.quarter_start}</td>
                </tr>
                <tr>
                  <td>Quarter End</td>
                  <td>{analyticsData.quarter_end}</td>
                </tr>
                <tr>
              ...
              <thead>
                <tr>
                  <th colSpan="2">Monthly Breakdown</th>
                </tr>
              </thead>
              <tbody>
                {analyticsData &&
                  Object.entries(analyticsData.monthly_breakdown).map(
                    ([month, count]) => (
                      <tr key={month}>
                        <td>{month}</td>
                        <td>{count}</td>
              ...
              <tbody>
                {analyticsData &&
                  Object.entries(analyticsData.requests_by_priority).map(
                    ([priority, count]) => (
                      <tr key={priority}>
                        <td>{priority}</td>
                        <td>{count}</td>
              ...
              <tbody>
                {analyticsData &&
                  Object.entries(analyticsData.requests_by_status).map(
                    ([status, count]) => (
                      <tr key={status}>
                        <td>{status}</td>
                        <td>{count}</td>
