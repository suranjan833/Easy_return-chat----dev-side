...
const ChatPopup = ({ user, onClose, initialPosition, index }) => {
  ...
  useEffect(() => {
    ...
    console.log(
      "[ChatPopup] 🎯 direct.selectUser exists:",
      !!direct?.selectUser,
    );
    console.log("[ChatPopup] 🎯 direct.ME_ID:", direct?.ME_ID);
    console.log(
      "[ChatPopup] 📊 direct.users count:",
      direct?.users?.length || 0,
    );

    if (direct?.selectUser && user?.id) {
      console.log(
        `[ChatPopup] ✅ Calling selectUser(${user.id}, ${user.conversation_id || user.pairKey || "null"})`,
      );
  ...
  useEffect(() => {
    if (
      direct?.users?.length > 0 &&
      user?.id &&
      direct?.activeConversationId === null
    ) {
  ...
  // ── unread divider visibility ──
  useEffect(() => {
    setShowUnreadDivider(
      (direct?.messages || []).some((m) => m.type === "unread_divider"),
    );
  }, [direct?.messages]);
  ...
  useEffect(() => {
    ...
    const isMyLatestMessage =
      lastMessage &&
      Number(lastMessage.sender_id) === Number(meId) &&
    ...
    requestAnimationFrame(() => {
      ...
      if (isFirstLoad) {
        setTimeout(() => {
          if (unreadDividerRef.current) {
            container.scrollTo({
              top: Math.max(0, unreadDividerRef.current.offsetTop - 80),
              behavior: "auto",
            });
          } else {
            container.scrollTo({
      ...
      // NEW MESSAGE
      else if (isMyLatestMessage || (!showUnreadDivider && isNewMessage)) {
        container.scrollTo({
          top: container.scrollHeight,
          behavior: "smooth",
        });
  ...
  }, [direct?.messages, meId, showUnreadDivider]);
  ...
  const renderContentWithLinks = (content) => {
    ...
    // Regex to match URLs with http/https, www, or plain domains
    const urlRegex =
  ...
  const renderMessage = (msg) => {
    if (msg._divider) return null;

    if (msg.type === "unread_divider") {
      return showUnreadDivider ? (
        <div
          key={msg.id || "unread-divider"}
          id="unread-divider"
      ...
      >
        ...
        >
          ...
          <div
            style={{
              maxWidth: "78%",
              padding: isEmojiOnly ? "4px" : "8px 12px",
          ...
          >
            {/* Forwarded label */}
            {(msg.type === "forward_message" ||
              msg.type === "forward_to_dm" ||
              msg.forwarded ||
              msg.is_forwarded ||
              ...
              !msg.is_deleted && (
                ...
                >
                  <i
                    ...
                    style={{ fontSize: "10px", transform: "scaleX(-1)" }}
                  />
                  <span>
                    Forwarded
                    {msg.source_system
                      ? ` from ${msg.source_system === "dm" ? "DM" : msg.source_system === "group_message" ? "Group" : msg.source_system === "group_reply" ? "Group Reply" : msg.source_system}`
                      : ""}
                  </span>
                </div>
              )}
            ...
            {msg.attachment && !msg.is_deleted ? (
              ...
              <span
                style={{
                  fontSize: isEmojiOnly ? "36px" : "13px",
                  lineHeight: isEmojiOnly ? "1.2" : "1.4",
                ...
                {isMe &&
                  (msg.read ? (
                    <i
                      ...
                      style={{ fontSize: "12px", color: "#a8d8ff" }}
                    />
                  ) : msg.delivered ? (
                    <i
                      className="bi bi-check2-all"
          ...
          {!msg.is_deleted && (
            <div className="popup-msg-actions">
              <button
                className="popup-msg-action-btn"
                onClick={() => direct?.handleReply(msg)}
                title="Reply"
              >
              ...
              <button
                className="popup-msg-action-btn"
                onClick={() => setForwardModal({ show: true, message: msg })}
                title="Forward"
              >
              ...
              {isMe && (
                <>
                  <button
                    className="popup-msg-action-btn"
                    onClick={() => direct?.startEditing(msg)}
                    title="Edit"
                  >
                    <i className="bi bi-pencil" />
                  </button>
                  <button
                    className="popup-msg-action-btn popup-msg-action-delete"
                    onClick={() => {
  ...
  return (
    ...
    >
      ...
      <div className="chat-header" onMouseDown={handleMouseDown}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span className="chat-title" style={{ fontSize: "14px" }}>
      ...
      >
        {messages.filter((m) => !m._divider).map((m) => renderMessage(m))}
      ...
      <ForwardMessageModal
        ...
        recentForwardUsers={(direct?.recentChats || [])
          .map((c) => {
        ...
        onForward={(messageId, recipientId) =>
          direct?.forwardMessage?.(messageId, recipientId)
        }
        // FIXED
        groups={groupChat?.groups || []}
        onForwardToGroup={(msg, groupId) =>
          direct?.forwardMessageToGroup?.(msg, groupId)
        }
        currentGroupId={null}
      />
      {/* Reply preview */}
      ...
      {direct?.editingMessageId && (
        <div className="modern-edit-preview">
          ...
          <button
            className="modern-edit-preview-close"
            onClick={direct?.cancelEditing}
          >
            ✕
          </button>
        </div>
        ...
        >
          ...
          {showEmojiPicker && (
            ...
            >
              <EmojiPicker
                onEmojiClick={(e) => {
                  direct?.setEditContent((p) => p + e.emoji);
                  setShowEmojiPicker(false);
                }}
                width={280}
                height={350}
          ...
          >
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (f) {
                  const valid = AttechmentSizeLimit(f, e);
          ...
          <button
            ...
            style={{
              ...
              border: "none",
              background:
                !direct?.editContent?.trim() && !direct?.attachment
                  ? "#c7d2fe"
                  : "#818cf8",
              display: "flex",
              alignItems: "center",
